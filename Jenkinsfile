pipeline {
       ...
       environment {
              ...
              REMOTE_USER = "root" // 원격(spring) 서버 사용자
              REMOTE_HOST = "10.10.11.65" // 원격(spring) 서버 IP(public IP)
              REMOTE_DIR = "/root/deploy" // 원격 서버에 파일 복사할 경로
              SSH_CREDENTIALS_ID = "jenkins-rsa-key" // Jenkins 대시보드에 등록한 jenkins ssh key ID
       }
              ...
              stage('Git Checkout') {
                     ...
                     stage('Copy to Remote Server'){
                            ...
                            stage('Remove Docker Build & Deploy') {
                                   steps {
                                          sshagesnt (credentials: [environment.SSH_CREDENTIALS_ID]){
                                                 sh ""
